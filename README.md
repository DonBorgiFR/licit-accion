...
| `Incoop.ico` + `tools/crear_accesos_directos.py` | Icono y accesos directos | 🟢 Paso 7. Alta y baja idempotentes en escritorio y menú de inicio. Lo que se hace a mano no se documenta ni se reproduce. **Desde el 10.C** hay un tercer acceso, sólo en el menú de inicio: el del despertador. |
| `Preparar equipo.vbs` | Instalador de doble clic | 🟢 **Paso 10, bloque C.** Deja un PC listo sin escribir un comando: dependencias, base de datos, accesos directos, y un veredicto que **no sale de que no haya fallado nada, sino del healthcheck**. Vive en la carpeta del proyecto porque se ejecuta antes de que existan los iconos. |
| `Despertador.vbs` | Activar o desactivar la prospección nocturna | 🟢 **Paso 10, bloque C.** **Un solo icono, no dos**: la ventana empieza contestando *«¿está activo?»* y ofrece el botón contrario. Tras actuar **vuelve a leer el estado** en vez de dar por bueno que la orden funcionó. |
...
9. **Paso 9 — La Voz del Proceso Silencioso**: 🟢 **Completado y Validado el 2026-08-27.**
   ...
   * **Leer el rastro entero con un programa.** `src/rastro.py` traduce a una forma única las
   ...
   > `python tools/verificar_rastro_real.py`, que vigila que sigan **congeladas en 21**.
   >
   > 🚨 **Y el alcance intra-proceso no bastaba, aunque estuviera razonado con datos.** La corrida
   > 24 produjo dos roturas nuevas con la firma contraria —un evento del pipeline entre dos del
   > servidor—, porque **la API y el pipeline escriben a la vez todos los días**. Desde el
   > 2026-09-01 hay además un **cerrojo de fichero del sistema operativo** sobre
   > `pipeline.jsonl.lock`: `msvcrt.locking` en Windows, `fcntl.flock` fuera, tomado siempre
   > **después** del de módulo para que los hilos de un proceso hagan cola sin tocar el disco.
   > Medido: 2, 3 y 5 procesos escribiendo a la vez **no pierden ni un evento**, y el coste por
   > evento se queda en **0,556 ms**. Lo que corrigió la apuesta en horas en vez de en meses fue
   > haber escrito, antes de codificar, **qué medición la refutaría**.
